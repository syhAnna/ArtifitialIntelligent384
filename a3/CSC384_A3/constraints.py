...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        TableConstraint.__init__(self, name, scope, assignment)
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.getValue() in self._required:
                assignments += 1
        return self._lb <= assignments <= self._ub  # check here
        ...
        def valsNotEqual(l):
            ...
            for v in vals:
                if v in self._required:
                    assignments += 1
            return self._lb <= assignments <= self._ub
...
class AllDiffFilterConstraint(Constraint):
    '''All diff constraint between a set of variables not consider empty string'''
    def __init__(self, name, scope):
        Constraint.__init__(self,name, scope)
        self._name = "AllDiff_" + name
    ...
    def hasSupport(self, var, val):
        ...
        def valsNotEqual(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the all diff'''
            vals = [val for (var, val) in l if (val is not '')]
            return len(set(vals)) == len(vals)
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsNotEqual, valsNotEqual)
