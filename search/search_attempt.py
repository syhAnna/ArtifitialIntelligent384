...
def aStarSearch(problem, heuristic=nullHeuristic):
	...
	# open_set.push(start, 0 + heuristic(problem.getStartState(), problem))
	...
	while not open_set.isEmpty():
		...
		curr_heuristic = curr_item[1]
		candidate = {curr_heuristic: [curr_item]}
		potential_heuristic = curr_heuristic
		# check tie
		while not open_set.isEmpty() and potential_heuristic == curr_heuristic:
			potential_item = open_set.pop()
			potential_heuristic = potential_item[1]
			if potential_heuristic == curr_heuristic:
				if potential_heuristic in candidate:
					candidate[potential_heuristic].append(potential_item)
				else:
					candidate[potential_heuristic] = [potential_item]
			else:
				open_set.update(potential_item, potential_item[2])
		# find the smallest heuristic funciton in the dictionary of
		# heuristic: [list of item with the same heuristic]
		# pick arbitrary item in the list of smallest heuristic
		curr_item = candidate[min(candidate.keys())].pop()
		curr = curr_item[0]
		curr_path, curr_moves = curr
		# push other items back to the priority queue
		pushBack = []
		for item in candidate.values():
			pushBack += item
		for item in pushBack:
			open_set.update(item, item[2])
