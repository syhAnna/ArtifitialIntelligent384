...
class PriorityQueue:
    ...
    def update(self, item, priority):
        ...
        for index, (p, c, i) in enumerate(self.heap):
            if i == item:   # -> if item already in the priority queue, check priority
                if p <= priority:   # -> if current priority > previous priority -> current path cost more -> not put into the heap
                    break
                del self.heap[index]    # here current cost of the path is less than the previous cost of the path -> update to the lower cost path
