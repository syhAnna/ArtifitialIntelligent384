...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        counter_beleif = util.Counter()
        for curr_pos in self.beliefs:   # is self.belief a dictionary?
            if self.beliefs[curr_pos] > 0:
                self.setGhostPosition(gameState, curr_pos)
                curr_gost = self.getPositionDistribution(gameState)
                for nxt_pos in self.legalPositions:
                    counter_beleif[nxt_pos] += self.beliefs[curr_pos] * curr_gost[nxt_pos]
        counter_beleif.normalize()
        self.beliefs = counter_beleif
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        temp = [x for x in self.legalPositions]
        count = 0
        while len(self.particles) < self.numParticles:
            self.particles.append(temp[count])
            count += 1
            if count == len(temp):
                count = 0
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        possibility = util.Counter()

        for pos in self.particles:
            if noisyDistance is None:
                possibility[self.getJailPosition()] = 1.0
            else:
                distance = util.manhattanDistance(pos, pacmanPosition)
                possibility[pos] += emissionModel[distance]
        curr_particles = []
        for _ in range(self.numParticles):
            if possibility.totalCount() != 0:
                curr_particles.append(util.sample(possibility))
            else:
                curr_particles.append(random.choice(self.legalPositions))
        self.particles = curr_particles
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        curr_belief_state = util.Counter()
        for pos in self.particles:
            curr_belief_state[pos] += 1
        curr_belief_state.normalize()
        return curr_belief_state
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        legal_pos = self.legalPositions
        permutation = list(itertools.product(legal_pos, repeat=self.numGhosts))

        # random shuffle the permutations
        random.shuffle(permutation)

        while particles >= len(permutation):
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            for ghosts in ghost_jail:
                particle = self.getParticleWithGhostInJail(particle, ghosts)

            probability = 1

            for i in range(self.numGhosts):
                if i not in ghost_jail:
                    emission = emissionModels[i]
                    dist = util.manhattanDistance(particle[i], pacmanPosition)
                    probability *= emission[dist]
            new_particles[particle] += probability

        self.beliefs = new_particles

        if new_particles.totalCount() == 0:
            self.initializeParticles()
        else:
            self.beliefs.normalize()
            for i in range(len(self.particles)):
                new_pos = util.sample(self.beliefs)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                distribution = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghost,
                                                                 self.ghostAgents[ghost])
                newParticle[ghost] = util.sample(distribution)
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        for particles in self.particles:  # convert all particles to util
            belief_distribution[particles] = belief_distribution[particles] + 1.0
